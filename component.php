...
if( $this->StartResultCache() ) {
    ...
    while ($arElement = $res->Fetch() ){
        ...
        if( is_array($arParams["PROPERTIES"]) ) {
            foreach ($arParams["PROPERTIES"] as $property){
                if ( strpos($property, "PICTURE") === false && strpos($property, "GALLERY") === false && strpos($property, "FILE") === false) {
                    ...
                    elseif( !empty($arElement["PROPERTY_".$property."_DESCRIPTION"]) )
                        $arFieldsResult[$property] =[
                            "DESCRIPTION" => $arElement["PROPERTY_" . $property . "_DESCRIPTION"],
                            "VALUE" => $arElement["PROPERTY_" . $property . "_VALUE"]
                        ];
